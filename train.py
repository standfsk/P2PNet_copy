...
def main(args):
    ...
    run_log_name = os.path.join(args.save_path, 'run_log.txt')
    ...
    train_set, val_set = loading_data.loading_data(os.path.join('data', args.data))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        checkpoint_latest_path = os.path.join(args.save_path, 'latest.pth')
        torch.save({
            'epoch': epoch,
        ...
        if epoch % args.eval_freq == 0 and epoch != 0:
            ...
            if abs(np.min(mae) - result[0]) < 0.01:
                checkpoint_best_path = os.path.join(args.save_path, 'best_model.pth')
                torch.save({
                    'epoch': epoch,
                    'model': model_without_ddp.state_dict(),
